...
public struct TopReposView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            WidgetHeaderView(label: "TOP REPOS", dotColor: .colorAccentGreen, timestamp: "stars")
                ...
                ForEach(Array(props.repos.enumerated()), id: \.offset) { _, repo in
                    HStack(spacing: 8) {
                        Text("\(repo.rank)")
                            ...
                            .foregroundStyle(.colorAccentGreen.opacity(0.5))
                        ...
                        HStack(spacing: 2) {
                            ...
                            Text("\(repo.stars)").font(.system(size: 9, design: .monospaced)).foregroundStyle(.colorTextMuted)
                        }
                    }
                    .padding(.vertical, 3)
                }
