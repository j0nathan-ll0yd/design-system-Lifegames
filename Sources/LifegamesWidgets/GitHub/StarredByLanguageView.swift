...
public struct StarredByLanguageView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            WidgetHeaderView(label: "STARRED BY LANGUAGE", dotColor: Color.colorAccentPink, timestamp: "starred")
                        ...
                        ForEach(Array(group.repos.enumerated()), id: \.offset) { _, repo in
                            HStack {
                                ...
                                HStack(spacing: 2) {
                                    ...
                                    Text(formatStars(repo.stars)).font(.system(size: 8)).foregroundStyle(Color.colorTextMuted)
                                }
                            }
                        }
                    }
