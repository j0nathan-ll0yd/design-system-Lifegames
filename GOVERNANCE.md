...
This boundary extends to the web runtime: the data/transport/orchestration layer (fetch, polling, WebSocket, service-worker update nudges) is **consumer-owned**, not part of the DS. The DS ships only the presentational hydration that consumes it (the `updaters*` DOM patchers, `adapters` transforms, `*-init` island entrypoints) plus the display-contract types they require (e.g. `runtime/poll-types.ts`). See ADR `0005`.
