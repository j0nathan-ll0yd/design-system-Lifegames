...
function flattenTokens(obj, path = [], type = null) {
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (val && typeof val === 'object' && '$value' in val) {
      ...
      tokens.push({path: [...path, key], $type: tokenType, $value: val.$value, $description: val.$description, $deprecated: val.$deprecated})
...
const SWIFT_FONT_WEIGHT = {300: 'Light', 400: 'Regular', 500: 'Medium', 600: 'SemiBold', 700: 'Bold'}

// --- Helpers ---

function hexToRgb(hex) {
  const h = hex.replace('#', '')
  return {r: parseInt(h.substring(0, 2), 16) / 255.0, g: parseInt(h.substring(2, 4), 16) / 255.0, b: parseInt(h.substring(4, 6), 16) / 255.0}
}
...
function emitTokensCss() {
  ...
  css += '\n  /* Spacing (fluid) */\n'
  for (const t of tokens) {
    if (t.$type !== 'dimension' || t.path[0] !== 'space') {
      continue
    }
  ...
  const cssFontFamily = (value) => String(value).includes(',') ? String(value) : `'${value}', sans-serif`
  ...
  for (const t of tokens) {
    ...
    const val = t.resolvedValue
    if (Array.isArray(val)) {
      css += `  ${toCssVar(t.path)}: cubic-bezier(${val.join(', ')});\n`
    }
  }
  ...
  for (const t of tokens) {
    ...
    const cssShadow = layers.map((l) => `${l.offsetX || 0}px ${l.offsetY || 0}px ${l.blur || 0}px ${l.spread || 0}px ${l.color}`).join(', ')
  ...
  for (const t of tokens) {
    ...
    if (val.fontSize) {
      css += `${prefix}-font-size: ${val.fontSize};\n`
    }
    if (val.fontWeight !== undefined) {
      css += `${prefix}-font-weight: ${val.fontWeight};\n`
    }
    if (val.lineHeight !== undefined) {
      css += `${prefix}-line-height: ${val.lineHeight};\n`
    }
    if (val.fontFamily) {
    ...
    if (val.letterSpacing) {
      css += `${prefix}-letter-spacing: ${val.letterSpacing};\n`
    }
  }
...
// tokens.d.ts — the declaration file `package.json`'s `.` export has always
// pointed `types` at but nothing ever emitted (atlas decision 0020; the export
// -surface Level-2 gate reads a declared-but-absent target as INDETERMINATE,
// never as "no surface"). It is generated from the SAME `jsonTokens` tree that
// produces tokens.js, so the two cannot drift: one model, two emitters.
//
// The shape is `declare const` + `export default`, mirroring tokens.js's single
// default export. Types are LITERAL and `readonly` — the emitted object is a
// frozen-by-convention constant table, so `tokens.color.pink[500]` types as
// `"#ff006e"`, not `string`. That is what the token model actually says, and
// widening it would throw away the only interesting thing a token type carries.

function emitTokensDts(jsonTokens) {
  return '// Generated by Style Dictionary — do not edit\ndeclare const tokens: ' + dtsType(jsonTokens, '') + ';\nexport default tokens;\n'
}

// A key needs quoting unless it is a plain JS identifier. Numeric token ramp
// keys ("500") and hyphenated role keys ("tool-use") are the common cases.
const DTS_BARE_KEY = /^[A-Za-z_$][A-Za-z0-9_$]*$/

function dtsType(value, indent) {
  if (Array.isArray(value)) {
    if (value.length === 0) {
      return 'readonly []'
    }
    const inner = indent + '  '
    return 'readonly [\n' + value.map((item) => inner + dtsType(item, inner)).join(',\n') + '\n' + indent + ']'
  }
  if (value !== null && typeof value === 'object') {
    ...
    const lines = keys.map((key) => {
      const name = DTS_BARE_KEY.test(key) ? key : JSON.stringify(key)
      return inner + 'readonly ' + name + ': ' + dtsType(value[key], inner) + ';'
    })
    return '{\n' + lines.join('\n') + '\n' + indent + '}'
  }
  // Leaf: a string or number token value, emitted as its literal type.
...
function colorsetContents(components) {
  return (
    JSON.stringify({
      colors: [
        {
          color: {'color-space': 'srgb', components: {red: components.red, green: components.green, blue: components.blue, alpha: components.alpha}},
          idiom: 'universal'
        }
      ],
...
function emitXcassetsColorsets() {
  ...
  for (const t of tokens) {
    ...
    const assetName = t.path.map((seg) => seg.replace(/([A-Z])/g, '-$1').toLowerCase().replace(/^-/, '')).join('-')
...
function emitSpacingSwift() {
  ...
  for (const t of tokens) {
    ...
    if (!isNaN(num)) {
      swiftSpacing += `    public static let s${key}: CGFloat = ${num}\n`
    }
  }
  swiftSpacing += '}\n'
  return swiftSpacing
...
function emitAISurfacesSwift() {
  ...
  for (const t of aiSurfaceTokens) {
    ...
    if (typeof val === 'string' && val.startsWith('#')) {
      ...
      if (swift) {
        ai += `    public static let ${name} = ${swift}\n`
      }
    }
  }
  ai += '}\n'
...
function emitDesignMd() {
  ...
  for (const t of tokensOfType('shadow')) {
    ...
    const summary = layers.map((l) => `${l.offsetX || 0}px ${l.offsetY || 0}px ${l.blur || 0}px ${l.spread || 0}px ${l.color}`).join(' / ')
    md += `| \`${fmtRef(t)}\` | \`${summary}\` |\n`
  }
  md += '\n'
  md += '## Component Tokens\n\n'
  ...
  for (const t of tokens) {
    if (
      ['color', 'space', 'motion', 'typography', 'iosTypography', 'shadow', 'font'].includes(t.path[0])
    ) {
      continue
    }
    if (t.path[0]) {
      componentRoots.add(t.path[0])
    }
  }
  ...
  if (existsSync(manifestPath)) {
    ...
    md +=
      `Single-purpose UI surfaces sharing a common dark/neon aesthetic. ${manifest.widgets.length} widgets across ${categories.length} categories. Each widget consumes a JSON fixture and renders identically on web (Astro) and iOS (SwiftUI).\n\n`
...
function emitShadcnCss() {
  ...
  for (const t of tokens) {
    tokenMapForShadcn.set(t.path.join('.'), t)
  }
  ...
  for (const t of tokens) {
    if (
      t.path[0] === 'color' && t.path[1] === 'surface' && ['thinking', 'citation', 'tool-use', 'artifact-frame', 'code-block'].includes(t.path[2])
    ) {
...
function emitDeprecatedTokensJson() {
  const deprecatedTokens = tokens.filter((t) => t.$deprecated === true).map((t) => ({
    path: t.path,
  ...
  platforms: {
    'web/css': {
      ...
      files: [
        ...
        {destination: 'tokens.d.ts', format: 'lifegames/tokens.d.ts'},
    ...
    'ios/xcassets': {buildPath: xcassetsDir + '/', files: [{destination: 'Contents.json', format: 'lifegames/xcassets-root'}]},
    'ios/swift': {
      buildPath: swiftDir + '/',
      files: [
...
const tokensDtsOut = emitTokensDts(jsonTokens)
...
const buildReportFiles = [
  ...
  {path: 'tokens.d.ts', bytes: Buffer.byteLength(tokensDtsOut, 'utf-8')},
...
sd.registerFormat({name: 'lifegames/tokens.d.ts', format: () => tokensDtsOut})
...
console.log('  DTS:      packages/tokens/dist/tokens.d.ts')
