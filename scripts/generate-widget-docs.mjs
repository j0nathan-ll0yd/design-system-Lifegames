...
function toKebab(name) {
  return name.replace(/V(\d+)$/, '-v$1').replace(/([a-z0-9])([A-Z])/g, '$1-$2').replace(/([A-Z])([A-Z][a-z])/g, '$1-$2').toLowerCase()
}
...
function resolveActualFilename(category, manifestName, ext) {
  ...
  if (fs.existsSync(path.join(catDir, exact))) {
    return manifestName
  }
  ...
  if (fs.existsSync(path.join(COMPONENTS_DIR, exact))) {
    return manifestName
  }
  return null
}
...
function resolveActualDir(category, manifestName, ext) {
  ...
  const files = fs.readdirSync(catDir).filter((f) => f.endsWith(ext))
  if (files.find((f) => f.startsWith(manifestName))) {
    return catDir
  }
  if (fs.existsSync(path.join(COMPONENTS_DIR, exact))) {
...
function parseTopLevelFields(body) {
  ...
  for (const line of body.split('\n')) {
    ...
    if (
      depth === 0 && (trimmed.endsWith(';') || trimmed.endsWith('[];') || trimmed.endsWith('}[];') || trimmed.endsWith('}'))
    ) {
  ...
  return fields.map((f) => {
    ...
    return {name, optional: optional === '?', type, nestedShape: isNested ? rawType.trim().replace(/;$/, '') : null}
  }).filter(Boolean)
}
...
function buildAlternativesSection(widget, manifest) {
  ...
  const siblings = manifest.widgets.filter((w) => w.category === category && w.name !== name && !productionNames.has(w.name))
...
`
  ...
  const DATA_SOURCES = {
    ...
    PinnedRepos: {
      ...
      api:
        'GraphQL viewer { pinnedItems(first:6, types:REPOSITORY) { nodes { ... on Repository { name description stargazerCount forkCount primaryLanguage { name color } url } } } }',
...
function cleanOrphans(manifest, outputDir) {
  ...
  let deleted = 0
  for (
    const category of fs.readdirSync(outputDir).filter((f) => {
...
function main() {
  ...
  const catCounters = {}
  for (const w of manifest.widgets) {
    if (!catCounters[w.category]) {
      catCounters[w.category] = 0
    }
  ...
  const widgetsToProcess = SINGLE_WIDGET
    ? manifest.widgets.filter((w) => {
      ...
      const kebab = toKebab(actualName || w.name)
      return (
        kebab === SINGLE_WIDGET || w.name === SINGLE_WIDGET || (actualName && toKebab(actualName) === SINGLE_WIDGET)
      )
    })
    : manifest.widgets
  ...
  const countSummary = Object.entries(categoryCounts).map(([cat, count]) => `${count} ${cat}`).join(', ')
