...
const PRODUCTION_WIDGETS_PATH = path.join(
  ROOT,
  'Sources/LifegamesWidgets/Resources/production-widgets.json',
);
...
function buildAlternativesSection(widget, manifest) {
  ...
  const siblings = manifest.widgets.filter(
    (w) => w.category === category && w.name !== name && !productionNames.has(w.name),
  );
...
<div style="margin: var(--space-20) 0;">
  ...
  if (isProduction) {
    ...
    const a11yMap = {
      DevActivityLog:
        'Renders semantic list markup; keyboard navigation supported via arrow keys and Enter.',
      StarredRepoList:
        'List items are keyboard accessible; external links open in new tab with proper aria-label.',
      MovementRings:
        'Apple Health-style activity rings with accessible numeric values for move/exercise/stand goals.',
      HeartRate:
      ...
      NightSummary:
        'Sleep metrics presented with text labels and percentages; no color-only information.',
      Workouts: 'Activity types and metrics rendered as accessible text with numeric values.',
      BioTerminal:
        'Terminal-style output rendered as semantic markup; cursor simulation is visual only.',
      ComingSoon: 'Static placeholder with descriptive heading and semantic markup.',
      IdentityCard:
        'Profile links open externally with proper aria-labels; compact mode uses CSS media queries.',
      ExplorationOdometerV3:
        'Large numeric displays with accessible labels; odometer effect is visual enhancement.',
      PlaceLeaderboardV3:
        'Ranked list with clear row headers; visit count and duration render as accessible text.',
      DndOverlay: 'Fullscreen overlay; closes via Escape key and maintains focus management.',
      FocusOverlay:
        'Fullscreen overlay with keyboard dismissal; hidden from screen readers (uses inert attribute).',
      SystemStatus:
        'Status indicators use text labels in addition to color; compact mode adapts to viewport.',
      ...
      ReadingFeed:
        'Article titles and sources render as accessible text; timestamps are semantic time elements.',
      TheatreReviews:
        'Review cards use semantic article markup; ratings display as numeric text and star glyphs.',
    };

    const a11yText =
      a11yMap[actualName] ||
      'Renders semantic markup with standard browser accessibility features.';
...
`;
  ...
  const DATA_SOURCES = {
    ...
    CommitLog: {
      schema:
        'packages/schemas/authored/dashboard-github.schema.json -- devActivity array (filtered to type: "commit")',
    ...
    CommitTimeline: {
      schema:
        'packages/schemas/authored/dashboard-github.schema.json -- devActivity array, plus DS-local repoColor enrichment',
    ...
    LanguageBars: {
      schema:
        'packages/schemas/authored/dashboard-github.schema.json -- languages map (bytes per language, transformed to [{name, pct, color}] by aggregation layer)',
    ...
    LanguageStack: {
      schema:
        'packages/schemas/authored/dashboard-github.schema.json -- languages map (same source as LanguageBars)',
    ...
    PinnedRepos: {
      schema:
        'packages/schemas/authored/widget-pinned-repos.schema.json (per-widget schema -- not covered by aggregate dashboard-github.schema.json)',
    ...
    WeeklyPulse: {
      schema:
        'packages/schemas/authored/dashboard-github.schema.json -- weeklyCommits array (raw number[], transformed to [{total, label}] with DS-local label and maxWeek)',
...
function main() {
  ...
  const widgetsToProcess = SINGLE_WIDGET
    ? manifest.widgets.filter((w) => {
        ...
        return (
          kebab === SINGLE_WIDGET ||
          w.name === SINGLE_WIDGET ||
          (actualName && toKebab(actualName) === SINGLE_WIDGET)
        );
      })
    : manifest.widgets;
