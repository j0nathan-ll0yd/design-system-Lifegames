...
for (const file of swiftWidgetFiles) {
  ...
  lines.forEach((text, idx) => {
    for (const f of FORBIDDEN_IMPORTS) {
      if (f.re.test(text)) {
        findings.push({file: path.relative(ROOT, file), line: idx + 1, label: f.label, text: text.trim(), blocking: true})
      }
    }
  })
}
...
console.log(`Scanned components: ${swiftComponentFiles.length} .swift files under Sources/LifegamesComponents{,Core}/`)
