...
import { canonicalize, buildMatrix, sortFindings } from './audit-widget-matrix.mjs';
...
describe('buildMatrix — MovementRings-style registry gap', () => {
  ...
  const sources = {
    ...
    consumersDoc: {
      consumedWidgets: ['MovementRings', 'HeartRate'],
      widgets: [],
  ...
  test('flags a HIGH finding when manifest says production but the registry has no entry', () => {
    ...
    const hit = findings.find(
      (f) =>
        f.id === 'MovementRings' &&
        f.severity === 'high' &&
        /production-widgets\.json.*no entry/.test(f.message),
    );
    assert.ok(
      hit,
      `expected a HIGH registry-gap finding for MovementRings, got: ${JSON.stringify(findings, null, 2)}`,
    );
  });
  ...
  test('does not raise the same finding for a widget present in both sources', () => {
    ...
    const falsePositive = findings.find(
      (f) => f.id === 'HeartRate' && /no entry for it on any platform/.test(f.message),
    );
  ...
  test('flags the self-inconsistency: consumedWidgets references a widget missing from widgets[]', () => {
    const { findings } = buildMatrix(sources);
    const hit = findings.find(
      (f) => f.id === 'MovementRings' && /self-inconsistent/.test(f.message),
    );
...
describe('buildMatrix — naming variants', () => {
  test('merges View-suffixed (Swift) and bare (web) spellings into one row, no spurious finding', () => {
    ...
    assert.equal(
      rows.length,
      1,
      'expected View-suffix and bare spelling to merge into one canonical row',
    );
    assert.equal(rows[0].id, 'BioTerminal');
    const spuriousVariantFinding = findings.find(
      (f) => f.id === 'BioTerminal' && /naming variant/.test(f.message),
    );
    assert.equal(
      spuriousVariantFinding,
      undefined,
      'web/Swift name pairing is by design, not drift',
    );
  });
  ...
  test('flags a genuine V3-suffix spelling mismatch between sources', () => {
      ...
      manifestWidgets: [
        {
          ...
          production: true,
          fixturePath: 'location/place-leaderboard-v3.json',
        },
      ],
      consumersDoc: {},
      docsInventoryWidgets: [],
    ...
    const hit = findings.find(
      (f) =>
        f.id === 'PlaceLeaderboard' && f.severity === 'info' && /naming variant/.test(f.message),
    );
...
describe('buildMatrix — docs/widget-inventory.json hasStory vs filesystem', () => {
  test('flags a mismatch when the registry field disagrees with the storybook filesystem probe', () => {
    ...
    const { findings } = buildMatrix(sources);
    const hit = findings.find(
      (f) => f.id === 'MovementRings' && /hasStory:false.*hasStorybookStory:true/.test(f.message),
    );
...
describe('buildMatrix — widget-consumers.json staleness', () => {
  test('flags generatedAt older than the 14-day threshold', () => {
    ...
    const expectedDays = Math.round(
      (nowMs - Date.parse(sources.consumersDoc.generatedAt)) / 86_400_000,
    );
    const hit = findings.find(
      (f) => f.id === '(widget-consumers.json)' && f.message.includes(`${expectedDays} days old`),
    );
    assert.ok(
      hit,
      `expected a staleness finding mentioning ${expectedDays} days old, got: ${JSON.stringify(findings)}`,
    );
  });
...
describe('buildMatrix — fixture triad', () => {
  test('flags missing empty/populated-max fixture variants', () => {
    ...
    const { findings } = buildMatrix(sources);
    const hit = findings.find(
      (f) => f.id === 'Widget' && /missing fixture variant/.test(f.message),
    );
    assert.ok(hit);
...
describe('buildMatrix — clean fixture set', () => {
  test('produces zero findings when every source and filesystem probe agrees', () => {
    ...
    assert.deepEqual(
      findings,
      [],
      `expected no findings for a fully consistent fixture set, got: ${JSON.stringify(findings, null, 2)}`,
    );
  });
});
