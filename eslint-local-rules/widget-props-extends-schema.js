    ...
    return {
      ...
      'Program:exit'() {
        if (!schemaImported) {
          ...
          const reportNode =
            ast.body.find((n) => n.type === 'ExportNamedDeclaration' || n.type === 'TSInterfaceDeclaration' || n.type === 'TSTypeAliasDeclaration') || ast
          context.report({node: reportNode, messageId: 'missingSchema', data: {name: filename.split('/').pop()}})
        }
      }
    }
  }
