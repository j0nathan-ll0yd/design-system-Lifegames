...
module.exports = {
  meta: {
    type: 'suggestion',
    docs: {description: 'D4: warn on references to deprecated Lifegames design tokens'},
    messages: {
      deprecatedCssVar: 'D4: CSS var {{var}} references a deprecated token. Check tokens/dist/deprecated-tokens.json for the replacement.',
      deprecatedSwiftAccess: 'D4: Swift accessor {{name}} references a deprecated token. Check tokens/dist/deprecated-tokens.json for the replacement.'
    },
    schema: []
  },
  create(context) {
    ...
    function checkStringValue(node, value) {
      ...
      while ((match = CSS_VAR_RE.exec(value)) !== null) {
        if (deprecatedCssVars.has(match[1])) {
          context.report({node, messageId: 'deprecatedCssVar', data: {var: match[1]}})
        }
      }
      ...
      while ((match = SWIFT_RE.exec(value)) !== null) {
        ...
        if (deprecatedSwiftNames.has(accessName)) {
          context.report({node, messageId: 'deprecatedSwiftAccess', data: {name: accessName}})
        }
      }
    }
