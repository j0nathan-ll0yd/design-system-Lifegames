...
tester.run('widget-props-extends-schema', rule, {
  valid: [
    ...
    {filename: outsideFile, code: 'export interface HeartRateProps { value: number; }'},
    // Completely unrelated file — rule is inert.
    {filename: nonTypesFile, code: 'export function foo() {}'}
  ],
  ...
  invalid: [
    ...
    {filename: widgetTypesFile, code: 'export type HeartRateProps = Record<string, unknown>;', errors: [{messageId: 'missingSchema'}]}
  ]
})
