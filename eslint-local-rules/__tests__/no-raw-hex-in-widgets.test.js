...
tester.run('no-raw-hex-in-widgets', rule, {
  valid: [
    ...
    {filename: outsideFile, code: 'const s = "color: #ff006e";'},
    // var(--lg-*, FALLBACK) exemption — second-arg hex is allowed
    {filename: widgetTsFile, code: 'const s = "color: var(--lg-color-accent-pink, #ff006e)";'},
    // Token-only reference — no raw hex
    {filename: widgetTsFile, code: 'const s = "color: var(--lg-color-text-title)";'},
    // Template literal with no hex
    {filename: widgetTsFile, code: 'const s = `color: var(--lg-color-bg-base)`;'},
    // Hex-shaped substring that is not a valid CSS hex length (5 digits) → skipped
    {filename: widgetTsFile, code: 'const s = "#12345";'},
    ...
    {filename: widgetTsFile, code: 'const s = "<span>&#9733;</span>";'},
    {filename: widgetTsFile, code: 'const s = "&#9474; &#8758; &#9472;";'}
  ],

  invalid: [
    // Raw 6-digit hex in string literal
    {filename: widgetTsFile, code: 'const s = "color: #ff006e";', errors: [{messageId: 'rawHex', data: {hex: '#ff006e'}}]},
    // Raw 3-digit hex in string literal
    {filename: widgetTsFile, code: 'const s = "color: #abc";', errors: [{messageId: 'rawHex', data: {hex: '#abc'}}]},
    // Raw 8-digit hex (with alpha) in string literal
    {filename: widgetTsFile, code: 'const s = "color: #ff006e80";', errors: [{messageId: 'rawHex', data: {hex: '#ff006e80'}}]},
    // Template literal raw hex
    {filename: widgetTsxFile, code: 'const s = `background: #06d6a0`;', errors: [{messageId: 'rawHex', data: {hex: '#06d6a0'}}]},
    ...
    {filename: widgetTsFile, code: 'const s = "var(--lg-color-accent-pink, #ff006e), #06d6a0";', errors: [{messageId: 'rawHex', data: {hex: '#06d6a0'}}]}
  ]
})
