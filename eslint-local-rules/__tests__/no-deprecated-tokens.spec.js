...
function buildTmpProject(deprecatedRecords) {
  ...
  fs.writeFileSync(path.join(root, 'packages/tokens/dist/deprecated-tokens.json'), JSON.stringify(deprecatedRecords))
  return root
}
...
// ── populated pipeline: one deprecated record ──────────────────────────────────
{
  const root = buildTmpProject([
    {cssVar: '--lg-color-legacy-pink', swiftName: 'Color.legacyPink', replacement: '--lg-color-accent-pink'}
  ])
  ...
  tester.run('no-deprecated-tokens [pipeline:populated]', rule, {
    valid: [
      ...
      {filename: widgetFile, code: 'const s = "color: var(--lg-color-accent-pink)";'}
    ],
    invalid: [
      ...
      {filename: widgetFile, code: 'const s = "LGColor.legacyPink";', errors: [{messageId: 'deprecatedSwiftAccess', data: {name: 'Color.legacyPink'}}]}
    ]
  })
...
// ── empty pipeline: deprecated-tokens.json is [] → rule silent ─────────────────
{
  ...
  tester.run('no-deprecated-tokens [pipeline:empty]', rule, {
    valid: [
      ...
      {filename: widgetFile, code: 'const s = "color: var(--lg-color-legacy-pink)";'}
    ],
    invalid: []
  })
