...
function scanText(text) {
  ...
  while ((m = HEX_RE.exec(text)) !== null) {
    ...
    if (isWithinSpan(m.index, exempt)) continue;
...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        'P1: web DS widgets must encode color via tokens, never raw hex literals (token-as-truth boundary)',
    },
    messages: {
      rawHex:
        'Raw hex `{{hex}}` forbidden in widget code. Use `var(--lg-color-*)` or add a token (see GOVERNANCE.md P1).',
    },
    schema: [],
  },
  create(context) {
