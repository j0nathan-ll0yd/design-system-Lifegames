...
async function writeJson(outPath, data) {
  ...
  const formatted = await prettier.format(JSON.stringify(data, null, 2), {...cfg, parser: 'json', filepath: outPath})
...
function makeOptionalsNullable(schema) {
  ...
  if (Array.isArray(schema)) {
    return schema.map(makeOptionalsNullable)
  }
    ...
    for (const [prop, propSchema] of Object.entries(result.properties)) {
      if (!required.has(prop) && typeof propSchema === 'object' && propSchema !== null) {
        if (propSchema.type && propSchema.type !== 'null' && !Array.isArray(propSchema.type)) {
          if (!propSchema.anyOf) {
            result.properties[prop] = {anyOf: [propSchema, {type: 'null'}]}
          }
        }
      }
    }
...
const MANUAL_SCHEMAS = {
        ...
        properties: {
          movement: {
            ...
            properties: {
              ...
              goals: {
                type: 'object',
                properties: {moveKcal: {type: 'number'}, exerciseMin: {type: 'number'}, standHr: {type: 'number'}, daylightMin: {type: 'number'}},
                required: ['moveKcal', 'exerciseMin', 'standHr', 'daylightMin']
              },
              solar: {
                type: 'object',
                properties: {sunriseHHmm: {type: 'string'}, sunsetHHmm: {type: 'string'}, currentProgressPct: {type: 'number'}},
  ...
  'og-image': {
    ...
    title: 'OGImageProps',
    type: 'object',
    properties: {avatarUrl: {type: 'string'}, name: {type: 'string'}, title: {type: 'string'}, quote: {type: 'string'}, experience: {type: 'string'}},
  ...
  'diagnostics-monitor': {
    ...
    properties: {
      ...
      counts: {type: 'array', items: {type: 'object', properties: {category: {type: 'string'}, count: {type: 'number'}}, required: ['category', 'count']}},
      fileSizeBytes: {type: 'number'},
      entries: {
        type: 'array',
        items: {
          type: 'object',
          properties: {id: {type: 'string'}, category: {type: 'string'}, timestamp: {type: 'string'}, message: {type: 'string'}, level: {type: 'string'}},
...
function generateEmptySchema(slug) {
  ...
  return {$schema: 'http://json-schema.org/draft-07/schema#', $id: `https://lifegames.org/schemas/widget/${slug}.json`, title, type: 'object'}
}
