...
function patchRefs(schema: object, schemaAbsPath: string): object {
  ...
  const patched = raw.replace(/"(\$ref)":\s*"([^"]+)"/g, (_match, key, ref) => {
    ...
    if (!filePart) {
      return `"${key}":"${ref}"` // pure JSON pointer, no file
    }
...
writeFileSync(brandedPath, await formatWithPrettier(brandedContent, brandedPath, 'typescript'), 'utf-8')
...
writeFileSync(indexPath, await formatWithPrettier(indexLines.join('\n'), indexPath, 'typescript'), 'utf-8')
...
for (const {relPath, name} of SCHEMA_ENTRIES) {
  ...
  try {
    execSync(`"${QUICKTYPE_BIN}" --src-lang schema --lang swift --top-level ${name} -o "${outFile}" "${absPath}"`, {stdio: 'pipe'})
...
// Re-run: pnpm -F @j0nathan-ll0yd/schemas codegen
//
...
for (let i = 0; i < swiftParts.length; i++) {
  // swiftParts is built 1:1 from SCHEMA_ENTRIES above, so both are defined here.
  const entry = SCHEMA_ENTRIES[i]
  const part = swiftParts[i]
  ...
  for (const line of lines) {
    if (!pastHeader) {
      if (
        line.startsWith('import Foundation') || (line.trim() === '' && contentLines.length === 0)
      ) {
...
for (let i = 0; i < rawSections.length; i++) {
  ...
  const {name: schemaName, contentLines} = section
  ...
  for (const typeName of typesInSection) {
    ...
    if (!typeRegistry.has(typeName)) {
      ...
      if (currentSig === first.signature) {
        ...
        if (!dropMap.has(i)) {
          dropMap.set(i, new Set())
        }
        dropMap.get(i)!.add(typeName)
        console.log(`  drop duplicate: ${typeName} (${schemaName} = ${rawSections[first.schemaIdx]?.name})`)
      } else {
        // Different struct — prefix with schema name
        const newName = `${schemaName}${typeName}`
        if (!renameMap.has(i)) {
          renameMap.set(i, new Map())
        }
...
const fixtureMap = {
  ...
  ds: existingMap.ds && typeof existingMap.ds === 'object' && Object.keys(existingMap.ds as object).length > 1
    ? existingMap.ds
    : {}
}
...
writeFileSync(fixtureMapPath, await formatWithPrettier(sortedStringify(fixtureMap), fixtureMapPath, 'json'), 'utf-8')
