...
const mapping = [
  {
    fixture: join(CONSUMER, 'data/profile.json'),
    schema: 'profile',
    label: 'consumer/data/profile.json',
  },
  {
    fixture: join(CONSUMER, 'data/system.json'),
    schema: 'system',
    label: 'consumer/data/system.json',
  },
  {
    fixture: join(CONSUMER, 'data/health.json'),
    schema: 'dashboard-health',
    label: 'consumer/data/health.json',
  },
  {
    fixture: join(CONSUMER, 'data/github.json'),
    schema: 'dashboard-github',
    label: 'consumer/data/github.json',
  },
  {
    fixture: join(CONSUMER, 'data/reading.json'),
    schema: 'dashboard-reading',
    label: 'consumer/data/reading.json',
  },
  {
    fixture: join(CONSUMER, 'data/books.json'),
    schema: 'dashboard-books',
    label: 'consumer/data/books.json',
  },
  {
    fixture: join(CONSUMER, 'data/theatre-reviews-sample.json'),
    schemaId: 'https://lifegames.dev/vendored/theatre-reviews-export.schema.json',
    label: 'consumer/data/theatre-reviews-sample.json',
  },
];
...
for (const m of mapping) {
  let data;
  try {
    data = JSON.parse(readFileSync(m.fixture, 'utf-8'));
  } catch (e) {
    results.push({ ...m, error: `READ_FAIL: ${e.message}`, mismatches: [] });
    continue;
  }
...
for (const f of dsFixtures) {
  ...
  if (!schema) {
    unmappedCount++;
    dsResults.push({ ...f, status: 'UNMAPPED' });
    continue;
  }
  let data;
  try {
    ...
    dsResults.push({ ...f, error: `READ_FAIL: ${e.message}` });
    continue;
  }
  const validate = ajv.getSchema(`urn:authored:${schema}`);
  if (!validate) {
    dsResults.push({ ...f, error: `SCHEMA_NOT_FOUND: ${schema}` });
    continue;
  }
  const ok = validate(data);
  dsResults.push({
    ...
    mismatches: ok
      ? []
      : validate.errors.slice(0, 10).map((e) => ({
          // cap per-fixture mismatches
          path: e.instancePath || '(root)',
          keyword: e.keyword,
          message: e.message,
        })),
  ...
  JSON.stringify(
    {
      ...
      ds: {
        results: dsResults,
        totalMismatches: dsCount,
        unmappedCount,
        totalFixtures: dsFixtures.length,
      },
      grandTotal: total,
