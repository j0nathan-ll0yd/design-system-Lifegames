...
const NAMESPACES: Namespace[] = [
  {
    name: 'identity',
    topLevelType: 'Identity',
    flatSchemaId: 'https://lifegames.org/schemas/copy.identity.flat.schema.json',
  },
  {
    name: 'widgets',
    topLevelType: 'Widgets',
    flatSchemaId: 'https://lifegames.org/schemas/copy.widgets.flat.schema.json',
  },
  {
    name: 'a11y',
...
for (const ns of NAMESPACES) {
  ...
  writeFileSync(
    join(SWIFT_TARGET, `${ns.topLevelType}.generated.swift`),
    genHeader(ns.name) + swift + (swift.endsWith('\n') ? '' : '\n'),
  );
...
for (const b of built) {
  console.log(
    `  dist/: ${b.name}.flat.schema.json, ${b.name}.flat.json, ${b.name}.ts, ${b.name}.zod.ts`,
  );
  console.log(
    `  swift: Sources/LifegamesCopy/${b.topLevelType}.generated.swift (+ Resources/${b.name}.en-US.json)`,
  );
}
