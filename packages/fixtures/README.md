...
import { fixtures, getDashboardFixture } from '@j0nathan-ll0yd/fixtures';
...
} from '@j0nathan-ll0yd/fixtures/post-adapter';

// Per-domain post-adapter map (variation → display shape):
import { profilePostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/profile';
import { healthPostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/health';
import { githubPostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/github';
import { readingPostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/reading';
import { booksPostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/books';
import { systemPostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/system';
import { starredReposPostAdapter } from '@j0nathan-ll0yd/fixtures/post-adapter/starredRepos';

// ── RAW (Playwright route interception) ─────────────────────────────────────
// Typed TS maps (domain → variation → raw export shape):
import { rawFixtures } from '@j0nathan-ll0yd/fixtures'; // all raw domains
import { healthVariations } from '@j0nathan-ll0yd/fixtures/raw/health'; // one domain

// Serialized JSON by path (what Playwright route handlers read):
//   @j0nathan-ll0yd/fixtures/generated/<kebab-domain>/<variation>.json
import baselineStarred from '@j0nathan-ll0yd/fixtures/generated/github-starred-repos/baseline.json';
```
...
- `profile`, `system`, `github`, `reading`, `health`, `books` are **authored display
  ...
  authored against `@j0nathan-ll0yd/schemas` `authored/`+`generated/` schemas.
...
```bash
pnpm -F @j0nathan-ll0yd/fixtures build      # generate + validate (fails on invalid)
pnpm -F @j0nathan-ll0yd/fixtures generate   # write src/generated + src/post-adapter JSON
pnpm -F @j0nathan-ll0yd/fixtures validate    # Ajv against portal-contract raw + @j0nathan-ll0yd/schemas
pnpm -F @j0nathan-ll0yd/fixtures test        # vitest (barrel + starredRepos determinism)
```
