...
| -------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **RAW** (pre-adapter)      | LP-export-shaped fixtures, typed from `@lifegames/portal-contract/schemas` | the web's Playwright **CloudFront route-interception** layer (`page.route(${CLOUDFRONT_BASE}/**)` reads these by file path) | committed JSON: `src/generated/<kebab-domain>/<variation>.json`; typed TS: `src/variations/*`          |
