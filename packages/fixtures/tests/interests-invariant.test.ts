...
function terminalInterests(variation: 'baseline' | 'full'): string[] {
  ...
  expect(promptIdx, `${variation}: expected a '$ ls -m interests/' prompt line`).toBeGreaterThanOrEqual(0)
...
function terminalSkills(variation: 'baseline' | 'full'): string[] {
  ...
  expect(promptIdx, `${variation}: expected a '$ printenv STACK' prompt line`).toBeGreaterThanOrEqual(0)
