...
function resolveNullableAnyOf(schema: JsonSchema): JsonSchema {
  if (!isNullableAnyOf(schema)) return schema;
...
function walkSchema(
  ...
  if (!schema.properties) return; // no named properties to enumerate
  ...
  for (const [key, propSchema] of Object.entries(schema.properties)) {
    ...
    if (!required.has(key)) {
      ...
      if (fixtureValue === undefined) {
        failures.push({
          path: childPath,
          domain,
          reason: `optional key "${key}" is missing (undefined) in full.json`,
        });
      }
    }
    ...
    if (required.has(key) && isNullableAnyOf(propSchema)) {
      ...
      if (fixtureValue === null) {
        failures.push({
          path: childPath,
          domain,
          reason: `nullable-but-required field "${key}" is null in full.json`,
        });
      }
    }
    ...
    const hasComplexPoly =
      resolved.oneOf !== undefined ||
      resolved.allOf !== undefined ||
      (resolved.anyOf !== undefined && !isNullableAnyOf(resolved));
    if (hasComplexPoly) {
      process.stderr.write(
        `[fixtures:check-full-coverage] WARN: ${domain}.${childPath} uses complex ` +
          `oneOf/allOf/anyOf — skipping (not a null-pattern)\n`,
      );
      continue;
    }
...
for (const [dir, schemaFile] of Object.entries(fixtureMap.raw)) {
  ...
  if (Object.prototype.hasOwnProperty.call(WALKER_EXCEPTIONS, dir)) {
    process.stdout.write(
      `[fixtures:check-full-coverage] SKIP ${dir} — ${WALKER_EXCEPTIONS[dir]}\n`,
    );
    continue;
  }
  ...
  if (!existsSync(fullPath)) {
    process.stderr.write(
      `[fixtures:check-full-coverage] ERROR: full.json not found: ${fullPath}\n`,
    );
    process.exit(1);
  }
...
if (zeroDomains.length > 0) {
  process.stderr.write(
    `\n[fixtures:check-full-coverage] FAIL: the following domain(s) produced zero ` +
      `checks — walker is a silent no-op for them:\n`,
  );
