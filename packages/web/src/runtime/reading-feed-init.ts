...
function paginateReadingFeed(card: HTMLElement, debug: boolean): void {
  ...
  if (!body) {
    if (debug) console.debug('[ReadingFeed] no .widget-body');
    return;
  }
  const list = body.querySelector('.article-list');
  if (!list) {
    if (debug) console.debug('[ReadingFeed] no .article-list');
    return;
  }
  const items = Array.from(list.querySelectorAll('.article-list-item'));
  if (debug) console.debug('[ReadingFeed] items =', items.length);
