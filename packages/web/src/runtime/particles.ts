export function startParticles(): void {
  ...
  // Bind the guarded values to explicitly-typed non-null consts so the narrowed
  // types flow into the nested closures below (resize/getGlow/frame), where
  // control-flow narrowing of the outer bindings is not preserved.
  ...
  function initParticles() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      particles.push({
        ...
        // Index is Math.floor(random * length) into a non-empty const array → in-bounds.
