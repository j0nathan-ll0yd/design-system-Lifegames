...
export function initTheatreReviews(
  ...
  fixtureData.reviews.forEach((r, i) => {
    ...
    html +=
      '<a class="theatre-card" href="' +
      esc(r.url) +
      '" target="_blank" rel="noopener noreferrer" style="animation-delay: ' +
      i * 0.08 +
      's">';
    ...
    if (r.imageUrl) {
      ...
      if (cardUrl) {
        const avifSrc = r.imageUrlCardAvif
          ? '<source srcset="' +
            esc(r.imageUrlCardAvif) +
            ' 1x, ' +
            esc(r.imageUrlAvif ?? r.imageUrl) +
            ' 2x" type="image/avif">'
          : '';
        const imgTag =
          '<img src="' +
    ...
    if (r.rating) {
      html +=
        '<span class="theatre-grade" style="color:' +
        gradeColor +
        ';border-color:' +
        gradeColor +
        '">' +
