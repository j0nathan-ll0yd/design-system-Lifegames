...
export function initTheatreReviews(
    ...
    if (r.imageUrl) {
      ...
      if (cardUrl) {
        const avifSrc = r.imageUrlCardAvif
          ...
          : '';
        const imgTag = '<img src="' + esc(cardUrl) + '" srcset="' + esc(cardUrl) + ' 1x, ' + esc(localSrc ?? r.imageUrl) + ' 2x" width="95" height="143" alt="' + esc(r.title) + '" loading="lazy" decoding="async" referrerpolicy="no-referrer"' + fallback + '>';
        ...
        const avifSrc = r.imageUrlAvif
          ...
          : '';
        const imgTag = '<img src="' + esc(localSrc ?? r.imageUrl) + '" width="95" height="143" alt="' + esc(r.title) + '" loading="lazy" decoding="async" referrerpolicy="no-referrer"' + fallback + '>';
        html += avifSrc ? ('<picture>' + avifSrc + imgTag + '</picture>') : imgTag;
      }
    }
    if (r.rating) {
