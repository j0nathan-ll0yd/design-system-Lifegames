...
export function updatePlaceLeaderboardV3(data: LocationExport): void {
  ...
  if (places.length === 0) {
    podiumEl.innerHTML =
      '<div style="text-align:center;color:var(--text-muted,#9ca3af);font-size:0.75rem;padding:16px 0;">' +
      esc(widgets.topPlaces.empty) +
      '</div>';
    listEl.innerHTML = '';
    card.classList.remove('is-loading');
    return;
  ...
  podiumEl.innerHTML = podium
    .map((place: TopPlace, i: number) => {
      ...
      return (
        '<div class="lb-v3-podium-row" data-rank="' +
        rank +
        '">' +
        '<span class="lb-v3-podium-rank" style="color:' +
        mc +
        ';">' +
        rank +
        '</span>' +
        '<div class="lb-v3-podium-info">' +
        '<span class="lb-v3-podium-name">' +
        esc(place.name) +
        '</span>' +
        podiumCatBadge +
        '</div>' +
        '<span class="lb-v3-podium-count" style="' +
        countStyle +
        '">' +
        place.visitCount +
        '</span>' +
  ...
  listEl.innerHTML = rest
    .map((place: TopPlace, i: number) => {
      ...
      const listCatBadge = place.category
        ? '<span class="lb-v3-list-cat" style="color:' +
          catColor +
          ';border-color:' +
          catColor +
          ';">' +
      ...
      return (
        '<div class="lb-v3-list-row" data-rank="' +
        rank +
        '">' +
        '<span class="lb-v3-list-rank">' +
        rank +
        '</span>' +
        '<span class="lb-v3-list-name">' +
        esc(place.name) +
        '</span>' +
        listCatBadge +
        '<span class="lb-v3-list-count">' +
        place.visitCount +
        '</span>' +
        '</div>'
      );
