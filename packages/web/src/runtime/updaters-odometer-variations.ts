...
export function updateExplorationOdometerV3(data: LocationExport): void {
  ...
  if (visits) {
    visits.textContent = data.totalVisits.toLocaleString()
  }
  if (places) {
    places.textContent = data.totalPlaces.toLocaleString()
  }
  if (cities) {
    cities.textContent = data.explorationStats.totalCities.toLocaleString()
  }
  if (states) {
    states.textContent = data.explorationStats.totalStates.toLocaleString()
  }

  if (city) {
    city.textContent = data.currentCity ? esc(data.currentCity) : ''
  }
