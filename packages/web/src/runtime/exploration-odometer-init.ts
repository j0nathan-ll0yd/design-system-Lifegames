...
export function initExplorationOdometerV3(container: HTMLElement, data: LocationExport): void {
  ...
  if (visits) visits.textContent = (data.totalVisits ?? 0).toLocaleString();
  if (places) places.textContent = (data.totalPlaces ?? 0).toLocaleString();
  if (cities) cities.textContent = (data.explorationStats?.totalCities ?? 0).toLocaleString();
  if (states) states.textContent = (data.explorationStats?.totalStates ?? 0).toLocaleString();
  if (city) city.textContent = data.currentCity ?? '';
