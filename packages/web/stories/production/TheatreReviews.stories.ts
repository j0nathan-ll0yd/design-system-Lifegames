...
const defaultReviews = [
  {
    ...
    rating: 'A',
    ratingNumeric: 9,
    excerpt: 'A sharp, witty examination of friendship tested by the purchase of an expensive white painting.',
  ...
  },
  {
    ...
    rating: 'A-',
    ratingNumeric: 8,
    excerpt: 'A raucous, genre-bending musical that brings an obscure historical figure to vivid life.',
  ...
  },
  {
    ...
    rating: 'A+',
    ratingNumeric: 10,
    excerpt: 'A masterful revival that captures the golden age of Broadway with infectious energy.',
...
const ratingColor = (n: number) =>
  n >= 9
    ? 'var(--lg-color-accent-green)'
    : n >= 7
    ? 'var(--lg-color-accent-amber)'
    : 'var(--lg-color-accent-red)'
...
function renderReviews(reviews: typeof defaultReviews) {
  ...
  return html`
    ...
    reviews.map((r) =>
      html`
            <div
              style="display: flex; gap: 10px; padding: 8px 0; border-bottom: 1px solid var(--lg-color-border);"
            >
              <div
                style="font-size: 1.3em; font-weight: bold; color: ${ratingColor(r.ratingNumeric)}; width: 32px; flex-shrink: 0;"
              >
                ${r.rating}
              </div>
              <div>
                <div style="font-size: 0.9em; font-weight: 600;">${r.title}</div>
                <div style="font-size: 0.78em; color: var(--lg-color-text-muted);">
                  ${r.excerpt}
                </div>
              </div>
            </div>
