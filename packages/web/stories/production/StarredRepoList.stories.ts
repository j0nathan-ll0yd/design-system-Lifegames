...
const defaultRepos = [
  ...
  },
  {
    ...
    name: 'astro',
    url: 'https://github.com/astro-community/astro',
    stars: 48000,
    language: 'TypeScript',
  ...
  return html`
    ...
    repos.map((r) =>
      html`
            ...
            >
              <div>
                <a href="${r.url}" style="color: var(--lg-color-accent-blue); font-size: 0.9em;"
                  >${r.owner}/${r.name}</a
                >
                <div style="font-size: 0.75em; color: var(--lg-color-text-muted);">
                  <span
                    style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: ${r.languageColor}; margin-right: 4px;"
                  ></span>
                  ${r.language} · ${r.starredAt}
                </div>
              </div>
              <span style="font-size: 0.8em; color: var(--lg-color-text-muted);"
