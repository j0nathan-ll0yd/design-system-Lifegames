...
import { initHeartRateInline, type HeartRateTestSeam } from '../../src/runtime/heart-rate-init';
...
function mountCanvas(opts: { withDataTest: boolean }): HTMLCanvasElement {
  ...
  if (opts.withDataTest) wrapper.setAttribute('data-test', '1');
...
describe('initHeartRateInline test seam', () => {
  beforeEach(() => {
    ...
    vi.spyOn(HTMLCanvasElement.prototype, 'getContext').mockImplementation(
      () => makeStubContext() as unknown as RenderingContext,
    );
