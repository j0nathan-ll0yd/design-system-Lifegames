...
function clickPage(page: number): void {
  const btn = document.querySelector(
    `.article-page-btn[data-page="${page}"]`,
  ) as HTMLButtonElement | null;
  if (!btn) throw new Error(`Missing page button ${page}`);
  btn.click();
}

function renderedTitles(): string[] {
  return Array.from(el('cardReading').querySelectorAll('.article-list-title')).map(
    (n) => n.textContent || '',
  );
}
