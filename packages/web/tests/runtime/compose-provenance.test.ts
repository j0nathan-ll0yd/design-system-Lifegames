...
describe('composeProvenance', () => {
  it('substitutes a single {phKey} with an anchor element', () => {
    const result = composeProvenance('Powered by {feedly} daily.', {
  ...
  it('is byte-identical to the prior markdown rendering for the Health body', () => {
    const result = composeProvenance(
      'Activity by my {watch} every ~15 minutes. Water powered by {water}. Caffeine powered by {coffee}.',
      {
  ...
  it('HTML-escapes the ref label', () => {
    const result = composeProvenance('{x}', {
      x: { label: 'A & B', href: 'https://example.com' },
    });
  ...
  it('HTML-escapes the ref href', () => {
    const result = composeProvenance('{x}', {
      x: { label: 'x', href: 'https://example.com?a=1&b=2' },
    });
  ...
  it('scheme guard: an http: href renders the escaped label as plain text, no anchor', () => {
    const result = composeProvenance('{x}', {
      x: { label: 'Click', href: 'http://example.com' },
    });
  ...
  it('scheme guard: a javascript: href renders the escaped label as plain text, no anchor', () => {
    const result = composeProvenance('{x}', {
      x: { label: 'Evil <script>', href: 'javascript:alert(1)' },
    });
  ...
  it('renders a body with empty refs unchanged (escaping only)', () => {
    expect(composeProvenance('Powered by a custom iOS App, hosted on AWS.', {})).toBe(
      'Powered by a custom iOS App, hosted on AWS.',
    );
  });
  ...
  it('leaves an unknown/absent placeholder as escaped literal text', () => {
    ...
    expect(
      composeProvenance('Powered by {mystery}.', { other: { label: 'x', href: 'https://x.io' } }),
    ).toBe('Powered by {mystery}.');
  });
