...
describe('updateHeartRate', () => {
  ...
  describe('paused state (watch.worn === false)', () => {
    ...
    it('data container .hr-data is always present in the DOM regardless of watch state', () => {
      const data: AdaptedHealth = {...makeHealth(), watch: {worn: false, since: null, source: 'hrGap'}}
      updateHeartRate(data)
    ...
    it('shows charging label when source is charging', () => {
      const data: AdaptedHealth = {...makeHealth(), watch: {worn: false, since: null, source: 'charging'}}
      updateHeartRate(data)
    ...
    it('removes is-paused on recovery (worn becomes true)', () => {
      // First call — paused
      const paused: AdaptedHealth = {...makeHealth(), watch: {worn: false, since: null, source: 'hrGap'}}
...
describe('updateWorkouts', () => {
  const workout: WorkoutEntry = {activityType: 'Outdoor Walk', duration: 3600, energyBurned: 400, distance: 5000, source: 'Apple Watch'}
...
describe('updateDevActivityLog', () => {
  ...
  it('renders empty state and clears stale content for empty events', () => {
    ...
    el('cardDevLog').querySelector('.widget-body')!.innerHTML = '<div class="gh-dal-terminal">stale baseline row</div>'
    updateDevActivityLog([])
...
describe('updateReadingFeed', () => {
  ...
  it('renders empty state and clears stale content for empty articles', () => {
    // SSR ships baseline (populated) content; the updater must replace it.
    el('cardReading').querySelector('.widget-body')!.innerHTML = '<ul class="article-list"><li>stale baseline article</li></ul>'
    updateReadingFeed([])
  ...
  it('renders pagination when articles > 10', () => {
    const manyArticles: AdaptedArticle[] = Array.from({length: 15},
...
describe('updateStarredRepos', () => {
  ...
  it('renders empty state and clears stale content for empty repos', () => {
    // SSR ships baseline (populated) content; the updater must replace it.
    el('cardStarredRepos').querySelector('.gh-starred-list')!.innerHTML = '<div class="gh-sl-row">stale baseline repo</div>'
    updateStarredRepos([])
...
describe('updateBookshelf', () => {
  ...
  describe('with widget-body DOM', () => {
    ...
    it('empty state contains correct title and body copy strings', () => {
      updateBookshelf(makeBooks({books: []}))
      expect(document.querySelector('#cardBooks .widget-empty-title')!.textContent).toBe(widgets.bookshelf.emptyTitle)
      expect(document.querySelector('#cardBooks .widget-empty-body')!.textContent).toBe(widgets.bookshelf.emptyBody)
    })
