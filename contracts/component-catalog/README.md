...
| `ratchet.mjs`                          | The conformance ratchet. Reads and writes the baseline, applies the gate predicate.                          |
| `conformance-baseline.json`            | Grandfathered conformance gaps. Generated by `check.mjs --update-baseline`.                                  |
| `check.mjs`                            | The gate. Five checks.                                                                                       |
| `schema.test.mjs`                      | Grammar/generator unit tests. Runs under `pnpm test:scripts`.                                                |
| `ratchet.test.mjs`                     | Ratchet known-answers. Runs under `pnpm test:scripts`.                                                       |
...
5. **Idempotence** — regenerate into a temp directory and compare bytes. A hand-edit to a committed
...
hook is DS's declared CI-parity tier so a drift someone else landed still blocks locally (B9).
`pnpm test:scripts` runs `schema.test.mjs` and `ratchet.test.mjs`.
...
is not debt anyone inherited. Close the gaps, or — only if the debt is genuinely unavoidable, and say
so in the PR body — re-record with `node contracts/component-catalog/check.mjs --update-baseline`.
