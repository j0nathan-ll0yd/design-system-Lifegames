...
for (const w of widgets) {
  ...
  w.hasSwiftMirror = swiftFiles.some((f) => f === `${idLower}props.swift` || f === `${idLower}view.swift`)
}
...
if (fs.existsSync(portfolioPages) || fs.existsSync(portfolioComponents)) {
  ...
  for (const f of astroFiles) {
    ...
    while ((match = importBlockRe.exec(src)) !== null) {
      ...
      for (const name of names) {
        ...
        if (!consumedByFile.has(name)) {
          consumedByFile.set(name, new Set())
        }
  ...
  if (!manifest) {
    if (CHECK_MODE) {
      ...
      console.warn('WARN: Portfolio repo not available and no widget-consumers.json fallback. ' + 'consumedByPortfolio will be false for all widgets.')
    }
  } else {
...
const curatedWidgets = existingManifest && Array.isArray(existingManifest.widgets) ? [...existingManifest.widgets] : []
